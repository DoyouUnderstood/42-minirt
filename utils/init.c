...
{
    *world = world_create();
    *camera = camera_create(900, 920, M_PI / 3);
...
void add_spheres(t_world *world) 
{
    ...
    obj_left->shape->transformation = matrix_multiply(matrix_translation(-2, 0.33, -1.75), matrix_scaling(0.33, 0.33, 0.33));
...
void world_fill(t_world *world) {
    ...
    obj_back_wall->shape->material.reflectiv = 0.0;
